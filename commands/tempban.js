...
module.exports = {
    run: async (message, args) => {
        ...
        message.channel.send(`**__${member.user.tag} a été banni pendant ${humanizeDuration(duration, {language: 'fr'})}__** :lock: !`)
        setTimeout(() => {
            message.guild.members.unban(member)
            message.channel.send(`**__${member.user.tag} a été débanni.__** :lock: `)
        }, duration)
    },
    name: 'tempban',
    guildOnly: true
